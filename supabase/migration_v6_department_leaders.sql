-- Liderança delegada de departamentos.

alter table public.people
  add column if not exists auth_user_id uuid references auth.users(id) on delete set null;

create unique index if not exists people_church_auth_user_unique
  on public.people(church_id, auth_user_id)
  where auth_user_id is not null;

alter table public.department_members
  add column if not exists can_manage boolean not null default false;

-- Vincula contas já existentes às pessoas pelo e-mail dentro de cada igreja.
...
where person.auth_user_id is null
  ...
  and 1 = (
    select count(*) from public.people duplicate
    where duplicate.church_id = person.church_id
      and lower(duplicate.email) = lower(person.email)
  );
...
as $$
  select exists (
    ...
    where member.department_id = target_department
      and member.active
      and member.can_manage
...
create policy "create departments" on public.departments
  for insert with check (
    public.has_church_role(church_id, array['super']::public.platform_role[])
  );
create policy "update departments" on public.departments
  for update using (
    public.has_church_role(church_id, array['super']::public.platform_role[])
    or public.manages_department(id)
  ) with check (
    public.has_church_role(church_id, array['super']::public.platform_role[])
    or public.manages_department(id)
  );
create policy "delete departments" on public.departments
  for delete using (
    public.has_church_role(church_id, array['super']::public.platform_role[])
  );

drop policy if exists "manage department roles" on public.department_roles;
create policy "manage department roles" on public.department_roles
  for all using (
    public.has_church_role(church_id, array['super']::public.platform_role[])
...
create policy "manage department members" on public.department_members
  for all using (
    public.has_church_role(church_id, array['super']::public.platform_role[])
    or public.manages_department(department_id)
  ) with check (
...
begin
  select exists(
    ...
    where id = target_department and church_id = target_church
  ) into department_exists;

  if department_exists then
    if not (
      public.has_church_role(target_church, array['super']::public.platform_role[])
      or public.manages_department(target_department)
    ) then
  ...
  insert into public.departments (
    id, church_id, name, department_type, description, active
  ) values (
    target_department, target_church, trim(department_name), department_type,
    nullif(trim(department_description), ''), true
  )
  ...
  insert into public.department_roles (
    church_id, department_id, title, sort_order
  )
  ...
  insert into public.department_members (
    church_id, department_id, person_id, role_id, active, can_manage
  )
  select
    target_church,
    target_department,
  ...
  from jsonb_to_recordset(coalesce(member_assignments, '[]'::jsonb))
    as assignment(person_id uuid, role_title text, can_manage boolean)
  join public.people person
    on person.id = assignment.person_id and person.church_id = target_church
  left join public.department_roles role
    on role.department_id = target_department and role.title = assignment.role_title;
